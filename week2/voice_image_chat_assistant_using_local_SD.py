...
def artist(city, seed=42):
    ...
    return image                  # ← must return image for Gradio
...
def talker(message):
    ...
    # Return as temp file path — Gradio Audio needs a file or bytes
    audio_path = "response.wav"
    with open(audio_path, "wb") as f:
        f.write(response.content)
    return audio_path             # ← return path, not raw bytes
...
ui.launch(inbrowser=True, auth=("nisum", "bananas"))